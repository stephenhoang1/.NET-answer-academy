...
export class Person implements IPerson {
  ...
  get palindrome(): boolean {
    ...
    var normalStr = `${this.fullName}`.toLowerCase().replace(/ /g,'')

      var reversedArr = Array.from(`${this.fullName}`.toLowerCase().replace(/ /g,'')).reverse

      var reversedStr = reversedStr.join('')
